...
local SF="NCv8.json"
local DEF={x=80,y=80,autoHop=false,autoEgg=true,targetEgg="Frostwyrm's Egg",checkInt=5,eggInt=15,fixLag=false,selectedRifts={},riftPriority="near",autoRift=false,riftDelay=3}
...
local function findRiftPrompt(m)for _,d in ipairs(m:GetDescendants())do if d:IsA("ProximityPrompt")and d.ActionText:lower():find("enter")then return d end end;return nil end
...
local function joinRift(arenaId)
    ...
    rStat.Text="🚶  Đến portal "..arenaId.."...";local ok,portal=tpToPortal(arenaId);if not ok then rStat.Text="❌  Không thấy portal!";return false end;task.wait(1.5)
    if portal then local prompt=findRiftPrompt(portal);if prompt then rStat.Text="🔑  Mở UI Rift...";if type(fireproximityprompt)=="function"then pcall(fireproximityprompt,prompt)end;task.wait(0.8)end end
    rStat.Text="👥  Tạo team...";pcall(function()VU2.DoRequest(DS2.ClientCreateTeam,arenaId)end);task.wait(0.8)
    rStat.Text="🤝  Join team...";pcall(function()VU2.DoRequest(DS2.ClientJoinTeam,arenaId)end);task.wait(0.8)
    rStat.Text="▶  Start...";pcall(function()VU2.DoRequest(DS2.ClientStartDungeon,arenaId)end);task.wait(1.5)
    if checkEntered(arenaId)then rStat.Text="✅  Vào Rift!";return true end
    rStat.Text="🔄  Retry...";pcall(function()VU2.DoRequest(DS2.ClientStartDungeon,arenaId)end);task.wait(1.5);return checkEntered(arenaId)
end
