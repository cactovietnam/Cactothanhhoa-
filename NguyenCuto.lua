...
local mSubStat=mkStat("hop","Để trống = mọi monster | Gõ để lọc nhanh danh sách")

-- Search box (full width, nổi bật hơn)
local mSrchRow=mkRow("hop",44)
local mSrchIcon=Instance.new("TextLabel",mSrchRow);mSrchIcon.Size=UDim2.new(0,30,1,0);mSrchIcon.Position=UDim2.new(0,8,0,0);mSrchIcon.BackgroundTransparency=1;mSrchIcon.Text="🔍";mSrchIcon.TextScaled=true;mSrchIcon.ZIndex=9
local mBox=Instance.new("TextBox",mSrchRow);mBox.Size=UDim2.new(1,-90,0,30);mBox.Position=UDim2.new(0,36,0.5,-15);mBox.BackgroundColor3=Color3.fromRGB(18,18,28);mBox.TextColor3=Color3.fromRGB(80,255,120);mBox.PlaceholderText="Tìm monster...";mBox.PlaceholderColor3=Color3.fromRGB(60,100,70);mBox.Text=TM;mBox.TextScaled=true;mBox.Font=Enum.Font.GothamBold;mBox.ClearTextOnFocus=false;mBox.BorderSizePixel=0;mBox.ZIndex=9
Instance.new("UICorner",mBox).CornerRadius=UDim.new(0,8);Instance.new("UIStroke",mBox).Color=Color3.fromRGB(60,180,80)
local mClearBtn=Instance.new("TextButton",mSrchRow);mClearBtn.Size=UDim2.new(0,32,0,30);mClearBtn.Position=UDim2.new(1,-42,0.5,-15);mClearBtn.BackgroundColor3=Color3.fromRGB(80,30,30);mClearBtn.TextColor3=Color3.fromRGB(255,120,120);mClearBtn.Text="✕";mClearBtn.TextScaled=true;mClearBtn.Font=Enum.Font.GothamBold;mClearBtn.BorderSizePixel=0;mClearBtn.ZIndex=9
Instance.new("UICorner",mClearBtn).CornerRadius=UDim.new(0,8)
local mStatLabel=mkStat("hop","")
mStatLabel.Text=(TM~=""and"🎯  Đang tìm: "..TM or"⬇  Chọn từ danh sách hoặc gõ tên")
mStatLabel.TextColor3=TM~=""and Color3.fromRGB(80,255,120)or Color3.fromRGB(100,100,130)

-- Gõ vào box → filter list real-time
mBox:GetPropertyChangedSignal("Text"):Connect(function()
    if mSettingBox then return end -- set từ code, bỏ qua
    applyFilter(mBox.Text)
end)
mBox.FocusLost:Connect(function(ep)
    -- Nếu xóa hết text → clear TM
    if mBox.Text==""then
        TM="";C.targetMon="";saveS(C)
        if mSelB then
            local prev=mSelB:GetAttribute("DisplayName")or""
            mSelB.BackgroundColor3=Color3.fromRGB(20,20,30);mSelB.TextColor3=Color3.fromRGB(185,185,205);mSelB.Text="    "..prev
            mSelB=nil
        end
        mStatLabel.Text="⬇  Chọn từ danh sách hoặc gõ tên";mStatLabel.TextColor3=Color3.fromRGB(100,100,130)
        mSubStat.Text="📋  Chưa chọn — sẽ hop khi không có bất kỳ monster nào"
    end
    if not ep then applyFilter("") end -- mất focus không phải Enter → reset filter hiển thị
end)
mClearBtn.MouseButton1Click:Connect(function()
    mSettingBox=true;mBox.Text="";mSettingBox=false;TM="";C.targetMon="";saveS(C)
    if mSelB then
        local prev=mSelB:GetAttribute("DisplayName")or""
        mSelB.BackgroundColor3=Color3.fromRGB(20,20,30);mSelB.TextColor3=Color3.fromRGB(185,185,205);mSelB.Text="    "..prev
        mSelB=nil
...
local mSelB=nil
local mSettingBox=false -- guard tránh filter khi set text từ code
...
local function mkMonItem(parent,rawName,displayName,accentColor)
    ...
    -- Lưu rawName và displayName vào attribute để filter
    b:SetAttribute("RawName",rawName)
    ...
    b.MouseButton1Click:Connect(function()
        ...
        TM=rawName;C.targetMon=TM;saveS(C)
        mSettingBox=true
        mBox.Text=rawName==""and"" or displayName
        mSettingBox=false
        mStatLabel.Text=(TM~=""and"🎯  Đang tìm: "..displayName or"⬇  Chọn từ danh sách hoặc gõ tên")
        mStatLabel.TextColor3=TM~=""and Color3.fromRGB(80,255,120)or Color3.fromRGB(100,100,130)
        mSubStat.Text=(TM~=""and"✅  Đã chọn: "..displayName or"📋  Chưa chọn — sẽ hop khi không có bất kỳ monster nào")
    end)
    return b
end

-- Filter list theo keyword
local function applyFilter(keyword)
    local kw=keyword:lower():gsub("^%s+",""):gsub("%s+$","")
    -- Filter cả 2 frame
    for _,sf in ipairs({mScanSF,mKnownSF})do
        for _,b in ipairs(sf:GetChildren())do
            if b:IsA("TextButton")then
                if kw==""then
                    b.Visible=true
                else
                    local dn=(b:GetAttribute("DisplayName")or""):lower()
                    local rn=(b:GetAttribute("RawName")or""):lower()
                    b.Visible=dn:find(kw,1,true)~=nil or rn:find(kw,1,true)~=nil
                end
            end
        end
    end
...
local function doScan()
    ...
    local seenDisplay={};local count=0
    for _,folder in ipairs({workspace:FindFirstChild("Monsters"),workspace:FindFirstChild("ClientMonsters")})do
        if folder then for _,o in ipairs(folder:GetDescendants())do
            if o:IsA("Model") then
                ...
                if nm or hm then
                    local dispName=getMonDisplayName(o)
                    -- Key dedup: ưu tiên tên hiển thị, fallback tên model
                    local key=dispName or o.Name
                    if not seenDisplay[key] then
                        seenDisplay[key]=true;count=count+1
                        -- rawVal: nếu có tên hiển thị → dùng keyword, không có → dùng tên model
                        local rawVal=dispName or o.Name
                        local label=dispName or o.Name
                        mkMonItem(mScanSF,rawVal,label,Color3.fromRGB(40,140,60))
                    end
                end
            end
        end end
...
-- Lấy tên hiển thị của 1 monster model (dùng chung scan + hasMon)
local function getMonDisplayName(o)
    -- 1. StringValue trực tiếp
    local svNames={"MonsterName","DisplayName","Name","monsterName","displayName","title"}
    for _,sn in ipairs(svNames)do
        local sv=o:FindFirstChild(sn)
        if sv and sv:IsA("StringValue") and sv.Value~="" then return sv.Value end
    end
    -- 2. BillboardGui > TextLabel
    for _,bb in ipairs(o:GetDescendants())do
        if bb:IsA("BillboardGui")then
            for _,tl in ipairs(bb:GetDescendants())do
                if tl:IsA("TextLabel") and tl.Text~="" and not tl.Text:find("^[%d%.]+$") then
                    return tl.Text
                end
            end
        end
    ...
    -- 3. Attribute
    local ok,attr=pcall(function()return o:GetAttribute("MonsterName")or o:GetAttribute("DisplayName")or o:GetAttribute("Name")end)
    if ok and attr and attr~="" then return tostring(attr) end
    -- 4. Fallback: tên model
    return nil
end
    ...
    for _,f in ipairs({workspace:FindFirstChild("Monsters"),workspace:FindFirstChild("ClientMonsters")})do
        if f then for _,o in ipairs(f:GetDescendants())do
            if o:IsA("Model")then
                ...
                if nm or hm then
                    if filter then
                        -- Match tên model HOẶC tên hiển thị
                        local rawMatch=o.Name:lower():find(filter,1,true)
                        local dispName=getMonDisplayName(o)
                        local dispMatch=dispName and dispName:lower():find(filter,1,true)
                        if not rawMatch and not dispMatch then continue end
                    end
                    -- Không check khoảng cách — tìm toàn server
                    return true
                end
            end
        end end
